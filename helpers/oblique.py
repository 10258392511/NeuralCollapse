...
def find_min_max_inner_prod(W: numpy.ndarray):
    inner = W.T @ W
    diag_inds = numpy.diag_indices(inner.shape[0])
    inner[diag_inds] = numpy.nan
    return numpy.nanmin(inner), numpy.nanmax(inner)


def solve_prob(loss_fn, solver, manifold, plot_loss=True, print_loss=False, temp_log_file="log_files/loss.txt"):
    """
    A wrap-up function for solving a manifold optimization problem.
    """
    assert isinstance(solver, SteepestDescent) or isinstance(solver, TrustRegions), "Choose RGD or RTR solver"
    prob = Problem(manifold=manifold, cost=loss_fn)
    if print_loss:
        Xopt = solver.solve(prob)
    else:
        with open(temp_log_file, "w") as wf:
            with redirect_stdout(wf):
                Xopt = solver.solve(prob)

    mode = "RGD" if isinstance(solver, SteepestDescent) else "RTR"
    data = convert_stdout_to_data(temp_log_file, mode=mode)
    if plot_loss:
