...
# check on function value is not enough
class SteepestDescentEarlyStop(Solver):
    """
    Steepest descent (gradient descent) algorithm based on
    steepestdescent.m from the manopt MATLAB package.

    Designed for strict-saddle functions where stopping criterion is relative loss function value change.
    """
    ...
    def solve(self, problem, x=None, reuselinesearch=False, eps=1e-6):
        """
        ...
        while True:
            ...
            if abs(objective(x_prev) - objective(x)) / abs(objective(x_prev)) < eps:
                stop_reason = f"Early stopping with relative objective value change less than {eps} (self-defined)"
                print(stop_reason)
                break
